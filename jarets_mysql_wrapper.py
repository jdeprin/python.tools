...
class Mysql(object):
	...
	"""
	Saving this for later I'll need the query.
	def db_add_tags(aws_instance_id, swarm_id, tag_name, tag_value):
		try:
			db = db_open()
			cur = db.cursor()
			cur.execute("INSERT INTO swarm_resource_tags(aws_resource_id,tag_name,tag_value,hmh_resource_id) VALUES (%s,%s,%s,%s)",
																													(aws_instance_id,
																													tag_name,
																													tag_value,
																													swarm_id))
	...
	def db_add_event(swarm_id,aws_resource_id,event_code,event_complete,event_description,event_start,event_end):
		# Must convert datetime from ISO to the expected format for django model type.
		import dateutil.parser
		event_start = dateutil.parser.parse(event_start)
		event_start = event_start.replace(tzinfo=None)
		
		if event_end is not None:
			event_end = dateutil.parser.parse(event_end)
			event_end = event_end.replace(tzinfo=None)
		
		try:
			db = db_open()
			cur = db.cursor()
			cur.execute("INSERT INTO swarm_resource_events(hmh_resource_id,aws_resource_id,event_type,event_complete,description,event_start,event_end) VALUES (%s,%s,%s,%s,%s,%s,%s)",
																																											(swarm_id,
																																											aws_resource_id,
																																											event_code,
																																											event_complete,
